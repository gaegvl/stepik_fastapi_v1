...
async def create_category(category:Annotated[CategoryCreate, Body()], db:Annotated[AsyncSession, Depends(get_db)]) -> CategorySchema:
...
async def update_category(category_id:Annotated[int, Path()], category:Annotated[CategoryCreate, Body()], db:Annotated[AsyncSession, Depends(get_db)]) -> CategorySchema:
    ...
    if category.parent_id is not None:
        result = await db.scalars(select(CategoryModel).where(and_(CategoryModel.id == category.parent_id, CategoryModel.is_active)))
        parent:CategoryModel|None = result.first()
        if not parent:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Parent category not found")
    
    await db.execute(update(CategoryModel).where(CategoryModel.id == category_id).values(is_active=True,**category.model_dump()))
    await db.commit()
...
async def delete_category(category_id:Annotated[int, Path()], db:Annotated[AsyncSession, Depends(get_db)]) -> dict:
    result = await db.scalars(select(CategoryModel).where(and_(CategoryModel.is_active, CategoryModel.id == category_id)))
    category:CategoryModel|None = result.first()
    if not category:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Category not found!")
    
    await db.execute(update(CategoryModel).where(CategoryModel.id == category.id).values(is_active=False))
    await db.commit()
